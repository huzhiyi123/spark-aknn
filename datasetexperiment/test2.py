...
import sys
sys.path.append("/home/shixiaoxiang/xuyaoheng/spark-aknn")
from utils import *
...
datapath="/home/shixiaoxiang/xuyaoheng/data/siftsmall/siftsmall/"
...
df1 =  kmeansPandasDfV1(data,k1=k1,traindatanum=2000,partitioncolname="partition")
...
def func2(df,repartitionres,k1,k2):
    dict_of_df = {k: v for k, v in df.groupby('partition')}
    res= []
    for i in range(k1):
        res.append([])
    for i in range(k2):
        for j in repartitionres:
            tmp = dict_of_df[j]['id'].values.tolist()
            res[i].append(tmp)
    return res
